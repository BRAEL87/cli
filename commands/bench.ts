...
import { SWEBenchAdapter } from '../swe-bench-adapter'
...
export function registerBenchCommand(program: Command): void {
    program
        ...
        .option('--work-dir <dir>', 'Working directory', join(tmpdir(), 'nova-swe-bench'))
        ...
        .action(async (options) => {
            ...
            try {
                // Load tasks
                const allTasks = await SWEBenchAdapter.loadTasks(options.tasks)
                ...
                // Create adapter
                const adapter = new SWEBenchAdapter(options.workDir, options.log)

                // Run evaluation
                const startTime = Date.now()
                const predictions = await adapter.runBatch(
                    tasks,
                    options.model,
                    parseInt(options.maxIterations),
                    parseInt(options.maxWorkers)
                )
