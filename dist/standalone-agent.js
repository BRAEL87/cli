...
exports.StandaloneAgent = void 0;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const axios_1 = __importDefault(require("axios"));
const loop_detector_1 = require("./loop-detector");
...
class StandaloneAgent {
    constructor(apiKey, apiBase = 'https://openrouter.ai/api/v1', logPath) {
        this.abortController = null;
        // Conversation memory: persists across prompts in REPL mode
        this.conversationHistory = [];
        // Async command registry: commandId → { process, output, done }
        this.asyncCommands = new Map();
        this.apiKey = apiKey;
        this.apiBase = apiBase;
        this.loopDetector = new loop_detector_1.LoopDetector();
        this.logger = new jsonl_logger_1.JSONLLogger(logPath);
    }
    ...
    async run(options) {
        const { prompt, maxIterations, model = 'tencent/hy3-preview:free', cwd = process.cwd(), timeout = 300000, onIteration, onAction, onObservation, onThought, onChunk, } = options;
        ...
        try {
            while (iteration < maxIterations) {
                ...
                if (!onIteration)
                    console.log(`Action: ${toolCall.tool}(${JSON.stringify(toolCall.args).substring(0, 50)}...)`);
                ...
                if (!onIteration)
                    console.log(`Result: ${success ? '✓' : '✗'} ${observation.substring(0, 100)}...`);
                ...
                if (loopCheck.isLooping) {
                    if (!onIteration)
                        console.log(`⚠️  Loop detected: ${loopCheck.reason}`);
            ...
            if (isTransient && iteration < maxIterations) {
                if (!onIteration)
                    console.error(`⚠ Network error (${error.message}), retrying iteration ${iteration}...`);
    ...
    async callAI(model, messages, timeout, onChunk, retries = 3) {
        ...
        for (let attempt = 1; attempt <= retries; attempt++) {
            try {
                if (isCompat) {
                    ...
                    const response = await axios_1.default.post(url, { model, messages, temperature: 0.7, stream: true }, {
                        headers: {
                            ...authHeaders,
                    ...
                    return await new Promise((resolve, reject) => {
                        ...
                        response.data.on('data', (chunk) => {
                            ...
                            for (const line of lines) {
                                const t = line.trim();
                                if (!t || t === 'data: [DONE]' || !t.startsWith('data: '))
                                    continue;
                                try {
                                    const token = JSON.parse(t.slice(6)).choices?.[0]?.delta?.content ?? '';
                                    if (token) {
                                        fullText += token;
                                        onChunk?.(token);
                                    }
                    ...
                    const r = await axios_1.default.post(`${this.apiBase}/ai/chat`, { model, messages }, { headers: { 'Authorization': `Bearer ${this.apiKey}` }, timeout });
                ...
                if (status === 401 || status === 403) {
                    const isPuter = this.apiBase.includes('puter.com');
                    if (isPuter)
                        throw new Error(`PUTER_AUTH: Puter requires authentication. Get your auth_token from puter.com → F12 → Application → Cookies → auth_token, then run /provider and paste it as the API key.`);
    ...
    async executeTool(tool, args, cwd, onChunk) {
        try {
            switch (tool) {
    ...
    toolReadChunk(filePath, startLine, endLine, cwd) {
        try {
            const full = path.resolve(cwd, filePath);
    ...
    toolWriteFile(filePath, content, cwd, onChunk) {
        try {
            const full = path.resolve(cwd, filePath);
    ...
    toolAppendFile(filePath, content, cwd) {
        try {
            const full = path.resolve(cwd, filePath);
            fs.mkdirSync(path.dirname(full), { recursive: true });
    ...
    toolEditFile(filePath, oldContent, newContent, cwd) {
        try {
            const full = path.resolve(cwd, filePath);
    ...
    toolMultiEdit(filePath, edits, cwd) {
        try {
            const full = path.resolve(cwd, filePath);
            let content = fs.readFileSync(full, 'utf-8');
            let applied = 0;
    ...
    toolMoveFile(source, destination, cwd) {
        try {
    ...
    // ─── Shell & Navigation ───────────────────────────────────────────────────
    async toolBash(command, cwd, onChunk) {
        if (!command)
            return { observation: 'bash requires a command argument', success: false };
        if (isDangerous(command))
            return { observation: `[BLOCKED] Dangerous command rejected: "${command}"`, success: false };
        ...
        return new Promise((resolve) => {
            ...
            exec(command, { cwd, timeout: timeoutMs, shell, maxBuffer: 1024 * 1024 * 5 }, (error, stdout, stderr) => {
    ...
    toolGrepInFile(filePath, pattern, cwd) {
        try {
            const full = path.resolve(cwd, filePath);
            const lines = fs.readFileSync(full, 'utf-8').split('\n');
    ...
    toolSearchCodebase(query, cwd) {
        ...
        const walk = (dir) => {
            ...
            try {
                const items = fs.readdirSync(dir);
                for (const item of items) {
                    if (item === 'node_modules' || item === '.git' || item === 'dist' || item === '__pycache__')
                        continue;
                    const full = path.join(dir, item);
                    try {
                        const stat = fs.statSync(full);
    ...
    async toolGetDiagnostics(filePath, cwd) {
        const full = path.resolve(cwd, filePath);
        ...
        if (ext === '.ts' || ext === '.tsx') {
            return new Promise(resolve => {
                ...
                if (!hasTsconfig && !hasLocalTsc) {
                    ...
                    exec(`node --check "${full}" 2>&1`, { cwd, timeout: 10000 }, (error, _, stderr) => {
                        resolve({
                ...
                exec(tscCmd, { cwd, timeout: 30000 }, (_, stdout) => {
        ...
        if (ext === '.py') {
            return new Promise(resolve => {
                const { exec } = require('child_process');
                exec(`python -m py_compile "${full}" 2>&1`, { cwd, timeout: 10000 }, (error, _, stderr) => {
                    if (error && stderr.includes('python: not found')) {
                        // Try python3
                        exec(`python3 -m py_compile "${full}" 2>&1`, { cwd, timeout: 10000 }, (err2, __, stderr2) => {
                            resolve({
    ...
    async toolRunTests(command, testFile, cwd, onChunk) {
    ...
    async toolSearchWeb(query) {
        if (!query)
            return { observation: 'search_web requires a query', success: false };
        try {
            // Use DuckDuckGo instant answer API — no key required
            const encoded = encodeURIComponent(query);
            const resp = await axios_1.default.get(`https://api.duckduckgo.com/?q=${encoded}&format=json&no_html=1&skip_disambig=1`, {
                timeout: 10000,
    ...
    async toolRunAsyncCommand(command, cwd) {
        if (!command)
            return { observation: 'run_async_command requires a command', success: false };
        if (isDangerous(command))
            return { observation: `[BLOCKED] Dangerous command rejected`, success: false };
        const commandId = `async-${Date.now()}`;
        const state = { output: '', done: false, exitCode: undefined };
        this.asyncCommands.set(commandId, state);
        const { exec } = require('child_process');
        const isWindows = process.platform === 'win32';
        const shell = isWindows ? 'cmd.exe' : '/bin/sh';
        const proc = exec(command, { cwd, shell, maxBuffer: 1024 * 1024 * 5 });
        proc.stdout?.on('data', (d) => { state.output += d; state.output = state.output.slice(-50000); });
        proc.stderr?.on('data', (d) => { state.output += d; state.output = state.output.slice(-50000); });
    ...
    toolCheckCommandStatus(commandId) {
        const state = this.asyncCommands.get(commandId);
        if (!state)
            return { observation: `No async command with id: ${commandId}`, success: false };
    ...
    clearCheckpoint(cwd) {
        try {
            fs.unlinkSync(this.getCheckpointPath(cwd));
        }
        catch { /* ignore */ }
    }
    // ─── Parsing & Prompts ────────────────────────────────────────────────────
    parseAllToolCalls(text) {
        ...
        while ((m = re.exec(text)) !== null) {
            try {
                ...
                if (name)
                    results.push({ name, tool: name, args: p.args || p.parameters || {} });
            }
            catch { /* skip */ }
        }
        if (results.length > 0)
        ...
        if (jm) {
            try {
                ...
                if (p.name)
                    results.push({ name: p.name, tool: p.name, args: p.args || {} });
            }
            catch { /* skip */ }
        }
        return results;
...
exports.StandaloneAgent = StandaloneAgent;
//# sourceMappingURL=standalone-agent.js.map