...
import { LoopDetectionState, IterationLog } from './types'
...
export class LoopDetector {
     ...
     */
    public detectLoop(log: IterationLog): { isLooping: boolean; reason?: string } {
        // Add to history
        ...
        if (exactLoop) return { isLooping: true, reason: exactLoop }

        // Check for semantic repetition (same tool, similar args)
        const semanticLoop = this.detectSemanticLoop()
        if (semanticLoop) return { isLooping: true, reason: semanticLoop }

        // Check for thought repetition
        const thoughtLoop = this.detectThoughtLoop()
        if (thoughtLoop) return { isLooping: true, reason: thoughtLoop }
     ...
     */
    private detectExactActionLoop(): string | null {
        if (this.state.actionHistory.length < this.state.repeatThreshold) return null
     ...
     */
    private detectSemanticLoop(): string | null {
        if (this.state.actionHistory.length < this.state.lastNActions) return null
        ...
        for (const [tool, count] of Object.entries(toolCounts)) {
            if (INVESTIGATION_TOOLS.has(tool)) continue
     ...
     */
    private detectThoughtLoop(): string | null {
        if (this.state.thoughtHistory.length < 4) return null
     ...
     */
    public getLoopBreakSuggestion(): string {
